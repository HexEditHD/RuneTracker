...
def PlayerDetails():
    ...
    runedata = get("https://apps.runescape.com/runemetrics/profile/profile?user=" + username + "&activities=20")
...
def Claninfo():
    ...
    data = read_csv("http://services.runescape.com/m=clan-hiscores/members_lite.ws?clanName=" + clanname, "utf-8", engine="python") 
...
def CombatLvlCalc():
    ...
    combatlvl = ((max(attstr, 2*magic, 2*ranged)) * 13/10 + defence + constitution + floor(prayer/2) + floor(summoning/2))/4
    HpDef = ceil((1 - (combatlvl % 1)) * 4)
    PraySumm = ceil((1 - (combatlvl % 1)) * 8)

    if attstr >= 2*magic and attstr >= 2*ranged:
        AttStr = ceil((1 - (combatlvl % 1)) / 0.325)
        Mage = ceil((attstr - magic * 2 ) / 2 + (1 - (combatlvl % 1 )) / 0.65)
        Range = ceil((attstr - ranged * 2) / 2 + (1 - (combatlvl % 1)) / 0.65)
    else:
        ...
        AttStr = max(ranged, magic) * 2 - attstr + ceil((1 - (combatlvl % 1)) / 0.325)

        # store this value in variable Mage first: assume mage-based combat first
        Mage = ceil(( 1 - (combatlvl % 1)) / 0.65 )
        if ranged > magic:
    ...
    print("\nYour current combat level is " + str(floor(combatlvl)) + ".\nFor level " + str(floor(combatlvl) + 1) + " you need one of the folowing:\n")
